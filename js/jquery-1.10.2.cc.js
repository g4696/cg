...
window.HurtCalc = new(function(){

	var kinds =
			{H:{H:3, B:1, P:3, U:5, F:3, I:2, A:3, M:4, D:3, Bo:5},
			B:{H:5, B:3, P:1, U:3, F:2, I:3, A:3, M:3, D:4, Bo:5},
			P:{H:3, B:5, P:3, U:1, F:3, I:4, A:2, M:3, D:3, Bo:5},
			U:{H:1, B:3, P:5, U:3, F:4, I:3, A:3, M:2, D:3, Bo:5},
			F:{H:3, B:4, P:3, U:2, F:3, I:1, A:3, M:3, D:5, Bo:5},
			I:{H:4, B:3, P:2, U:3, F:5, I:3, A:1, M:3, D:3, Bo:5},
			A:{H:3, B:3, P:4, U:3, F:3, I:5, A:3, M:1, D:2, Bo:5},
			M:{H:2, B:3, P:3, U:4, F:3, I:3, A:5, M:3, D:1, Bo:5},
			D:{H:3, B:2, P:3, U:3, F:1, I:3, A:4, M:5, D:3, Bo:5},
			Bo:{H:1, B:1, P:1, U:1, F:1, I:1, A:1, M:1, D:1, Bo:3}};
	...
	this.phyHurt = function(p, m){
		...
		effect.push( attrEffect( p.attrs, m.attrs ));
		effect.push( kindEffect( p.kind, m.kind ));
		effect.push( skillEffect_p( p.skill, p.skill_lv ));
		if(p.hasCritical){
			base_hurt += criticalEffect( p.lv, m.lv, m.def );
		}
		...
		total.upperHurt = Math.round(total.hurt * 1.1);
		total.lowerHurt = Math.round(total.hurt * 0.9);
		total.effect = effect;
		return total;
	};
	...
	this.magHurt = function(p, m){
		...
		if(p.skill != 12){
			base_hurt = m_hurt( p.skill, p.skill_lv );
			effect.push( mndDiffEffect( p.mnd, m.mnd ));
			effect.push( mndLevelEffect( p.mnd, p.skill_lv ));
			effect.push( attrEffect_m( p.skill, m.attrs ));
			effect.push( atkEffect_m( p.matk, p.skill_lv ));
		}
		else{
			base_hurt = s_hurt( p.mnd, m.def, p.skill_lv );

			if(p.hasCritical){
				base_hurt += criticalEffect( p.lv, m.lv, m.def );
			}

			effect.push( shkNumEffect( p.atkNum ) );
		}
	...
	function kindEffect(a_kind, b_kind){
		var effect = [1.3, 1.15, 1, 0.85, 0.7];
		return effect[kinds[a_kind][b_kind] - 1];
	}

	function kindWeaponEffect(a_kind, b_kind){
		var effect = [1.2, 1.1, 1, 0.9, 0.8];
		return effect[kinds[a_kind][b_kind] - 1];
	}

	function criticalEffect(a_lv, b_lv, b_def){
		return (a_lv * b_def) / (b_lv * 2);
	}

	function shkNumEffect( num ){
		var effect = [1, 0.88, 0.77, 0.66, 0.55];
		return effect[num - 1];
	}

	function mndDiffEffect( a_mnd, b_mnd ){
	...
	function mndLevelEffect( mnd, lv ){
	...
	function attrEffect_m( skill, attrs ){
		var mAttr = skill % 4;
		return 1 + ( ( attrs[(mAttr+1)%4] - attrs[(mAttr+3)%4] ) * 0.035);
	}

	function atkEffect_m( matk, lv ){
