var Monster = new (function(){
	...
	this.sim = function (ability, lv, rndBP, dropBP, addBP){	// ability : 寵物最高檔次
		var bp = [],
			rbp = [],
			m = {},
			result,
		...
		for(i=0; i<5; i++){
			if(ability.multi > 20){ // 灌水點數
				overflow.bp += calcBP2(ability.bp[i], rndBP[i], dropBP[i], 1, (ability.multi-20));
			}

			result = calcBP2(ability.bp[i], rndBP[i], dropBP[i], lv, ability.multi);
			result += addBP[i];
			bpGrow += params.dx[ability.bp[i]-dropBP[i]]; // BP成長率
			bp.push(result);
			rbp.push(Math.floor(result));
		}
		if(overflow.bp > 0){
			overflow.lv = overflow.bp / bpGrow;
		}
	...
	function calcBP2(bp, rndBP, dropBP, lv, multi){	//計算單項BP
		var result = 0,
			dx = params.dx;

		result = (bp + rndBP - dropBP) * multi / 100;	//等級1的單項BP
		if(lv != 1){
			result += dx[bp-dropBP]  * (lv - 1);
		}
